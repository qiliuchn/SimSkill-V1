...
### CRN's benefit is contingent on the vehicle population staying FIXED

The rho values above were measured over *signal plans*, where every design simulates an
identical set of vehicles and only the timing changes. When the design variable changes the
vehicle population itself, CRN can go **negative**: a transit frequency-allocation study
(`design-transit-service-plan-under-a-bus-hour-budget`) measured rho across six design pairs at
**+0.138, −0.028, −0.078, −0.271, −0.299, −0.450 — four of six negative**, because changing a
line's bus count changes the number and identity of bus vehicles, so SUMO consumes its `--seed`
stream in a different order and the random numbers are not actually *common* between designs.

So: before pairing on seed, ask whether your design change adds, removes or renames vehicles. If
it does, CRN pairing may *increase* the variance of the difference, and the CRN-paired column of
the resolvable-difference table above is optimistic. Fall back to independent seeds and the
independent column. See [[transit-network-design-and-frequency-setting]].
