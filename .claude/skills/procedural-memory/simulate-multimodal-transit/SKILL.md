...
- **...but a GENEROUS `until=` silently erases traffic delay.** The same mechanism bites from the other direction: SUMO departs a stop at `max(arrival + duration, until)`, so a timetable with any slack turns every bus into a schedule-adherent vehicle and congestion stops appearing in its measured round-trip time. Verified: a timetable built from a guessed bus speed gave a measured cycle of **1216 s with 10,827 background cars and 1216 s with none — bit-identical**; rebuilding it from an *uncongested buses-only* run gave 1010 s uncongested vs 1169 s congested (+15.7%). If the scenario is about bus running time, delay, or fleet size, calibrate the published timetable from a buses-only run first, then assert that congested and uncongested cycle times actually differ. See [[intermodal-transfer-and-person-stage-semantics-in-sumo]] and `design-transit-service-plan-under-a-bus-hour-budget`.
- **Persons never re-plan mid-trip, and stranded ones are easy to drop.** A traveller whose connection degrades just waits longer — the number of ride legs matched duarouter's a-priori plan 100% of the time even though 71–78% of riders boarded later than planned. Someone who never completes still gets a `<personinfo>` with `duration="-1"` and a leg with `vehicle="NULL"` but a **real `waitingTime`**; a parser that reads `-1` as a zero-duration completion counts them as free trips.
