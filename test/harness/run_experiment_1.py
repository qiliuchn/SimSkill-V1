"""Driver for Experiment #1 (test/experiments.md §4 "Experiment #1 (Core)"): the
condition × LLM grid that answers RQ1 (Accuracy), RQ2 (LLM), RQ3 (Memory Ablation),
...
for a second sweep over the same grid.

This version runs the grid strictly sequentially, one pair at a time, streaming that
pair's `run_experiment.py` output live as it happens. See `run_experiment_1_parallel.py`
for a variant that can run multiple pairs concurrently (trading live streaming for
buffered per-pair output).
...
python test/harness/run_experiment_1.py --conditions full-ver,vanilla-cc\
    --models claude-haiku-4-5 --verify-model claude-opus-5\
    --repeats 1 --keep-worktree

# Continue a previous sweep
python test/harness/run_experiment_1.py --conditions full-ver\
    --models deepseek-v4-pro --verify-model deepseek-v4-pro\
    --repeats 1 --keep-worktree --continue
    
python test/harness/run_experiment_1.py --conditions vanilla-cc\
...
python test/harness/run_experiment_1.py \
    --conditions full-ver,vanilla-cc --models claude-haiku-4-5 --repeats 1 --keep-worktree
    
# Phase 1.2: add the single-memory-type ablation conditions; same LLM, n=1
python test/harness/run_experiment_1.py \
    --conditions proc-mem-ver,sem-mem-ver,infer-frame-only \
    --models claude-haiku-4-5 --repeats 1 --keep-worktree

# Phase 1.3: repeat the headline comparison across more LLMs
python test/harness/run_experiment_1.py \
    --conditions full-ver,vanilla-cc --models claude-opus-5,deepseek-v4-pro,deepseek-v4-flash \
    --repeats 1 --keep-worktree
...
python test/harness/run_experiment_1.py --dry-run
...
# ========== run experiment for one (condition, model) pair ==========
def run_one_pair(condition: str, model: str, args: argparse.Namespace) -> dict:
    """Run one (condition, model) pair by shelling out to run_experiment.py with the
    right flags, streaming its output live. Returns a dict with the pair and the exit"""
    argv = [
        sys.executable,  # python
        str(RUN_EXPERIMENT_SCRIPT),  # invoke `run_experiment.py`
        "--condition", condition,
        "--model", model,
        "--all-tasks",
    ...
    start = time.time()
    print(f"\n\n\n=== condition={condition} model={model} ===", flush=True)
    # -----------------------------------------------
    # invoke run_experiment.py by subprocess
    # -----------------------------------------------
...
def main() -> None:
    ...
    conditions = (
        ...
        if args.conditions
            ...
            name
            for name in conditions_cfg
            if name != re1.VERIFY_CONDITION_NAME
        ]
    )
    ...
    print(f"repeats={args.repeats} cache_mode={args.cache_mode}" + (" [DRY RUN]" if args.dry_run else ""))
    ...
    if args.continue_mode:
        # Continue mode: do not clear experiment tasks  — just ensure it exists so appends work.
        print("Continuing experiment from existing experiment_tasks.jsonl file.")
        experiment_tasks_jsonl.touch()
    else:
        # Fresh run: clear/create the file.
        print("Fresh run: clearing experiment_tasks.jsonl file.")
        with experiment_tasks_jsonl.open("w", encoding="utf-8") as f:
            pass
    
    # NOTE: this is the core line of code to run all pairs
    # each pair will run in a separate worktree, and get a 
    # return a status returncode
    pairs_result = [run_one_pair(condition, model, args) for condition, model in pairs]
