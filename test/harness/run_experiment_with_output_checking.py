...
# Follow-up prompt used when a run finished WITHOUT writing its episodic-memory record
# (see invoke_one_task).  It is sent by resuming that same session rather than starting a
# fresh one — that is the whole point: the session still holds the task's own context, so it
# can write the record from what it actually did instead of re-deriving (or worse, re-running)
# the task.  Deliberately does not invoke any skill: under `strip_system_skills` conditions
# there is no /infer to call, and the record format doc lives at the repo (worktree) root in
# every condition, so this prompt works unchanged across the whole condition table.
EPISODIC_SAVE_RETRY_PROMPT = (
    "You did not save this simulation experience to episodic memory.\n\n"
    "If the task is still going on, continue running it.  If it is done, save the record.\n"
    "If the task is already done, just write the record from the work "
    "already done in this session, reusing the scripts, outputs and numbers you already "
    "produced.\n\n"
    "Create `episodic-memory/<timestamp>/` (timestamp format `YYYY-MM-DD_HH-MM-SS`) in this "
    "repository and populate it exactly as `episodic-memory-record-format.md` (at the repo "
    "root) specifies: `summary.md` with its full YAML frontmatter (task, timestamp, success, "
    "attempts, skills_used, knowledge_used, ingested, new_skills, new_pages, updated_skills, "
    "updated_pages) and its required sections, plus `attempts/attempt-N/` and `outputs/`.\n\n"
    "Report honestly: if the task did not succeed, set `success: false` — do not inflate the "
    "record.\n\n"
    "When you are done, reply with the absolute path of the `summary.md` you wrote."
)
...
def resume_session_to_save_episodic(
    *,
    worktree_path: Path,
    model_config: dict,
    condition: dict,
    ...
    """Re-enters the task's OWN session (`claude -p --resume <session_id>`) and asks it to
    write the episodic-memory record it failed to write the first time.

    This is a recovery path, not part of the measured task: a run whose record never landed
    is unusable downstream (no self-reported success/attempts/skills_used, and nothing for
    `verify` to inspect), so it is worth one cheap follow-up turn before writing the run off.
    Resuming rather than starting a fresh session is what makes it cheap AND faithful — the
    session still has the whole task in context, so it reports what it actually did instead
    of reconstructing it from the filesystem.

    Args:
        worktree_path: the worktree the task ran in — same cwd as the original invocation,
            so `episodic-memory/` and `episodic-memory-record-format.md` resolve identically.
        model_config: resolved models.yaml entry — same model that ran the task.
        condition: resolved conditions.yaml entry — only `safe_mode` matters here, and it
            must match the original invocation.
        settings_path: the worktree's settings.local.json, or None (as for the task run).
        session_id: the id of the session to resume.  Pass the id Claude Code itself reported
            in the task's JSON result when there is one; it is authoritative, whereas the
            harness-minted `--session-id` only matches it as long as Claude Code keeps the
            id it was handed.
    ...
    Returns:
        A dict describing this follow-up invocation (cmd/returncode/stdout/stderr/raw_result/
        timing).  It is carried on the task's raw result under "episodic_retry" and is
        accounted for SEPARATELY from the task's own cost/time — see annotate_with_cost_time.
        Whether the record actually appeared is decided by the caller re-running
        find_new_episodic_record, not by anything in here.
    """
    cmd = build_claude_command(
        task_text=EPISODIC_SAVE_RETRY_PROMPT,
        model_config=model_config,
        condition=condition,
        settings_path=settings_path,
        session_id=session_id,
        resume=True,  # NOTE: the whole point — same session, so the task is still in context
        max_budget_usd=max_budget_usd,
        skip_permissions=skip_permissions,
    )

    retry_start_wall = time.time()
    ...
    wall_ms_measured = int((time.time() - retry_start_wall) * 1000)
    ...
    try:
        ...
        print(
            "\n  [Episodic-save retry LLM usage]",
            json.dumps(raw_result.get("modelUsage", {}), indent=2),
        )
    except json.JSONDecodeError as exc:
        parse_error = str(exc)
        print("\n  [Episodic-save retry LLM usage] unavailable")

    if proc.returncode != 0:
        # Not fatal: the record may still have been written before claude exited non-zero,
        # so let the caller's find_new_episodic_record have the final say.
        print(
            f"\n  [Warning] Episodic-save retry exited with returncode={proc.returncode}; "
            "see the run's .episodic-retry.stderr.log"
        )

    return {
        "cmd": cmd,
        "resumed_session_id": session_id,  # the session id this retry resumed
        "returncode": proc.returncode,
        "stdout": proc.stdout,  # full, untruncated
...
def invoke_one_task(
    *,
    ...
    episodic_retry = None  # set below only if the first pass produced no record
    ...
    if episodic_summary:
        ...
        # ===== if claude code didn't generate an episodic record, we'll re-start the same session to ask claude code to save again=====
        print("\n  [Warning] Episodic memory record not found. Re-starting the same claude code session and ask Claude code to save it...")
        # Resume THIS task's session (not a fresh one) so the record is written from the
        # task's own context rather than reconstructed. Prefer the session id Claude Code
        # reported in its JSON result over the harness-minted one: they normally match, but
        # the reported id is what actually exists on disk to resume.
        resume_session_id = raw_result.get("session_id") or session_id
        print("\n  [Resuming session]", resume_session_id)
        if not isinstance(resume_session_id, str) or not resume_session_id:
            # Nothing resumable — no reported id and no minted id. Skip the retry rather
            # than launch a fresh, context-free session that could only invent a record.
            print("\n  [Warning] No session id available to resume — skipping the episodic-save retry.")
        else:
            episodic_retry = resume_session_to_save_episodic(
        ...
        # After the re-started session, try to find the episodic record again
        # NOTE: still measured from run_start_wall, so a record the retry wrote (which is
        # necessarily newer) is picked up just the same.
        episodic_summary, episodic = find_new_episodic_record(worktree_path, run_start_wall)
        
        if episodic_summary:
            episodic_path = episodic_summary.parent
            episodic_path_str = str(episodic_path)  # episodic memory record folder; absolute path; str
            print(f"\n  [Episodic memory record] recovered by the retry: {episodic_path_str}")
        else:
            print("\n  [Warning] Episodic memory record still not found! Please check the CLAUDE output.")
            episodic_path = None
    ...
    return {
        ...
        "episodic_retry": episodic_retry,  # the --resume follow-up that re-asked for the record, or None if it wasn't needed
        "task": task,
        "model_key": model_key,
    }
...
def annotate_with_cost_time(
    ...
    if verify_result is not None:
        ...
        verified_success = None 
        verification_agreement = None  # nothing to agree with — verification was skipped
    ...
    # ----- the episodic-save retry, when invoke_one_task had to fall back to one -----
    # Its tokens/time are reported here but deliberately kept OUT of `cost_usd` and
    # `wall_clock_ms`: it is harness-induced recovery overhead (the run didn't write its
    # record when first asked), not part of the task's own problem-solving budget, and
    # folding it in would make §3.2's cost/time comparison across conditions measure two
    # different things for the runs that needed it and the runs that didn't.
    retry = raw.get("episodic_retry")
    episodic_retry_info = {"attempted": False}
    if retry:
        retry_usage = (
            cost_time.parse_claude_json_result(retry["raw_result"])
            if retry["raw_result"]
            else cost_time.RunUsage()
        )
        retry_cost_usd, retry_cost_warnings = cost_time.compute_dollar_cost(
            retry_usage, model_key, price_table
        )
        episodic_retry_info = {
            "attempted": True,
            "recovered": episodic_summary_path is not None,  # did the record actually appear
            "resumed_session_id": retry["resumed_session_id"],
            "session_id": retry_usage.session_id,
            "returncode": retry["returncode"],
            "claude_json_parse_error": retry["raw_stdout_parse_error"],
            "start_time_utc": retry["start_time_utc"],
            "end_time_utc": retry["end_time_utc"],
            "wall_clock_ms_measured": retry["wall_clock_ms_measured"],
            "tokens": {
                "input": retry_usage.input_tokens,
                "output": retry_usage.output_tokens,
                "cache_creation": retry_usage.cache_creation_input_tokens,
                "cache_read": retry_usage.cache_read_input_tokens,
            },
            "cost_usd": retry_cost_usd,
        }
        if episodic_retry_info["recovered"]:
            episodic_warnings.append(
                "episodic-memory record was missing after the task run and was recovered by "
                "resuming the session — its cost/time are reported under 'episodic_retry', "
                "not in cost_usd/wall_clock_ms"
            )
        else:
            episodic_warnings.append(
                "episodic-memory record still missing after resuming the session to ask for "
                "it — see raw/<run_id>.episodic-retry.stdout.log"
            )
        episodic_warnings += [f"[episodic_retry] {w}" for w in retry_usage.warnings]
        episodic_warnings += [f"[episodic_retry] {w}" for w in retry_cost_warnings]
    ...
    return {
        ...
        "episodic_retry": episodic_retry_info,  # the --resume follow-up that re-asked for the record (attempted=False when it wasn't needed)
...
def write_raw_output(run_id: str, raw: dict, results_dir: Path) -> dict:
    ...
    # The episodic-save retry (invoke_one_task's --resume follow-up) is a second `claude -p`
    # invocation with its own output; keep it in its own log rather than concatenating it
    # onto the task's, so "what did the task itself print" stays answerable.
    retry = raw.get("episodic_retry")
    paths["episodic_retry_stdout"] = None
    paths["episodic_retry_stderr"] = None
    if retry:
        retry_stdout_path = raw_dir / f"{run_id}.episodic-retry.stdout.log"
        retry_stdout_path.write_text(retry.get("stdout") or "")
        print(f"\n  [Stdout] Wrote episodic-save retry stdout to {retry_stdout_path}")
        paths["episodic_retry_stdout"] = str(retry_stdout_path)

        retry_stderr = retry.get("stderr") or ""
        if retry_stderr:
            retry_stderr_path = raw_dir / f"{run_id}.episodic-retry.stderr.log"
            retry_stderr_path.write_text(retry_stderr)
            print(f"\n  [Stderr] Wrote episodic-save retry stderr to {retry_stderr_path}")
            paths["episodic_retry_stderr"] = str(retry_stderr_path)
    ...
    for repeat_idx in range(1, args.repeats + 1):
        ...
        for i, task in enumerate(tasks):
            ...
            try:
                ...
                result["episodic_retry_stdout_path"] = raw_paths["episodic_retry_stdout"]  # episodic-save retry stdout; absolute path; str (None if no retry)
                result["episodic_retry_stderr_path"] = raw_paths["episodic_retry_stderr"]  # episodic-save retry stderr; absolute path; str (None if no retry/empty)
                ...
                if result["episodic_summary_path"]:
                    if result["episodic_retry"]["attempted"]:
                        print(
                            "\n  [Episodic memory record] summary path (recovered by the "
                            f"episodic-save retry): {result['episodic_summary_path']}"
                        )
                    else:
                ...
                worktree_tasks_item = {
                    ...
                    "episodic_retry_attempted": result["episodic_retry"]["attempted"],  # did the run need a --resume follow-up to write its record
                    "episodic_retry_recovered": result["episodic_retry"].get("recovered"),  # ... and did that follow-up produce one (None if not attempted)
